...
import 'package:snc_cs_client/models/chat_model.dart';
...
class ChatProvider with ChangeNotifier {
  ...
  void toggleStatus(String ticketNo) {
    ...
    _statusPerTicket[ticketNo] = currentStatus == 'Solved'
        ? 'Unresolved'
        : 'Solved';
    notifyListeners();
  }
  ...
  Future<void> updateStatus(String ticketNo, String newStatus) async {
    ...
    try {
      ...
      notifyListeners();
      print(
        'DEBUG (ChatProvider): Status for ticket $ticketNo updated to $newStatus in Firestore.',
      );
    } catch (e) {
      print(
        'ERROR (ChatProvider): Failed to update status for ticket $ticketNo in Firestore: $e',
      );
